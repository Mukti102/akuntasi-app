        ...
        <div class="form-card rounded-2xl">
            <div class="px-8 py-6 border-b border-white/10">
                <h3 class="text-xl font-bold text-white">
                    <i class="fas fa-edit mr-3 text-blue-400"></i>Edit Asset
                </h3>
            </div>
            <div class="p-8">
                <form action="{{ route('assets.update', $asset->id) }}" class="grid grid-cols-2 gap-4" method="POST">
                    @csrf
                    @method('PUT')
                    <x-input-text label="Nama" type="text" value="{{ $asset->name }}" name="name"
                    ...
                    <x-input-text label="Jumlah" value="{{ $asset->quantity }}" type="number" name="quantity"
                        placeholder="Jumlah Semua Asset" />
                    <x-input-text label="Harga Satuan" value="{{ $asset->purchase_price }}" type="number"
                        name="purchase_price" placeholder="Masukkan Harga Persatuan" />
                    <x-input-text label="Lokasi" value="{{ $asset->location }}" type="text" name="location"
                        placeholder="Masukkan Lokasi Barang" />

                    <x-input-text label="Umur Ekonomis" value="{{$asset->umur_ekonomis}}" type="text" name="umur_ekonomis"
                        placeholder="Masukkan Umur Ekonomis" />
                    <x-input-text label="Penyusutan" value="{{$asset->penyusutan}}" type="text" name="penyusutan" placeholder="Masukkan Penyusutan" />

                    <x-input-text label="Tanggal" value="{{ $asset->purchase_date }}" type="date" name="purchase_date"
